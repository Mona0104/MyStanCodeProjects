...
def main():
	"""
	This program could use recursion concept to draw the triangle.
	"""
...
def sierpinski_triangle(order, length, upper_left_x, upper_left_y):
	...
	if order == 0:
		...
		triangle.add_vertex((upper_left_x+(1/2)*length, upper_left_y+length*0.866))
		window.add(triangle)
		sierpinski_triangle(order-1, length/2, upper_left_x, upper_left_y)
		sierpinski_triangle(order-1, length/2, upper_left_x + (1/2)*length, upper_left_y)
		sierpinski_triangle(order-1, length/2, upper_left_x+(1/4)*length, upper_left_y+(length/2)*0.866)
